...
MPI_Reduce(App, Ap, m, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
...
if (rank==0) {
	...
	while ( k < n ){
		...
		cblas_dgemv (CblasColMajor,CblasNoTrans, m, n/size, al , &A[intv_A*rank], m, &p[intv_x*rank], incx,  be, App, incy);
		MPI_Reduce(App, Ap, m, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
