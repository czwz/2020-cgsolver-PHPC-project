...
__global__ void kernel_mv(double *A, double *X, double *AA, int m) {
    int i,j,k;
...
void cgsolver( double *A, double *b, double *x, int m, int n ){
	...
	cudaMemcpy(d_b, b, sizeof(double)*n,   cudaMemcpyHostToDevice);
        ...
        cublasDgemv_v2(cublasHandle, CUBLAS_OP_N, m, n, &al, d_A, lda, d_x, incx, &be, d_Ap, incy); cudaDeviceSynchronize();
	...
	temp=-al;
        ...
        cublasDcopy_v2(cublasHandle, n, d_r, incx, d_p, incy); cudaDeviceSynchronize();
	...
	cublasDdot_v2(cublasHandle, n, d_r, incx, d_rt, incy, &rsold); cudaDeviceSynchronize();
        ...
        while ( k < n ){
		cublasDgemv_v2(cublasHandle, CUBLAS_OP_N, m, n, &al, d_A, lda, d_p, incx, &be, d_Ap, incy); cudaDeviceSynchronize();
		...
		temp = rsnew/rsold;
                ...
                cublasDcopy(cublasHandle, n, d_p, 1, d_pt, 1); cudaDeviceSynchronize();
		rsold = rsnew;
                k++;
        }
